					...
					<div class="row">
						<div class="col-md-8 article">
							<?php if(get_field('hide_content_title', $the_ID)) { ?>
								<?php echo ( get_field('show_content_title', $the_ID ) ? '<h2 class="article-title"><span class="faculty_department pre-title">' . courses_department_links($the_ID) . '</span>' . get_field('content_title', $the_ID ) . '<span class="edit-link">' . edit_post_link('Edit this faculty member profile', ' | ', '') . ' | <a href="' . add_query_arg(array('post_type'=>'faculty'),admin_url('post-new.php')) . ' title="Add new faculty member" class="post-add-link">Add new faculty member</a></span></h2>' : '' ); ?>
							<?php } else { ?>
